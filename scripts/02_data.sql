...
SELECT
    ROW_NUMBER() OVER (ORDER BY i.ISSUER_ID, s.DATE DESC) AS SEGMENT_ID,
    i.ISSUER_ID, i.TICKER, i.COMPANY_NAME, i.GICS_SECTOR,
    s.DATE AS PERIOD_END_DATE, s.VARIABLE_NAME AS SEGMENT_NAME,
...
WHERE s.DATE >= DATEADD(YEAR, -3, CURRENT_DATE()) AND s.VALUE IS NOT NULL AND s.VALUE > 0;
...
SELECT
    ...
    t.DATE, a.VARIABLE_NAME AS RATE_NAME, a.GEO AS COUNTRY, t.VALUE AS RATE_PCT,
...
SELECT
    ...
    t.DATE AS YEAR_DATE, a.VARIABLE_NAME AS INDICATOR_NAME, a.GEO AS COUNTRY,
